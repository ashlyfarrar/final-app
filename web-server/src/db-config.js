...
const { CREATE_COURSES_TABLE } = require('./queries/courses.queries');
...
const database = process.env.DB_DATABASE || 'gpa_calc';
...
(async () => {
  try {
    ...
    const coursesTableCreated = await query(conToDatabase, CREATE_COURSES_TABLE);

    if (userTableCreated && coursesTableCreated) {
...
const getConnection = async () => {
  if (!connectionInstance) {
    connectionInstance = await connectionToDatabase();
  }
